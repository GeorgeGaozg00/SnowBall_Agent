...
def get_following_posts_with_playwright(cookie_str, my_uid, max_scrolls=20):
    ...
    with sync_playwright() as p:
        ...
        def handle_response(response):
            url = response.url
            if any(keyword in url for keyword in ['timeline', 'statuses', 'friendships']):
                try:
    ...
    # 解析帖子数据
    print(f"\n解析API数据（共{len(api_responses)}个响应）...")
    seen_ids = set()
...
def main():
    ...
    # 获取关注人的帖子
    print("\n获取关注人的帖子...")
    posts = get_following_posts_with_playwright(cookie_str, my_uid, max_scrolls=20)
    ...
    for i, post in enumerate(posts[:10], 1):
        print(f"\n{i}. {post['author_name']}")
        title = post['title'][:40] if post['title'] else post['description'][:40]
        print(f"   {title}...")
