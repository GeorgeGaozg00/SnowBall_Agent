...
def get_post_detail_from_link(share_link, cookie_str):
    ...
    try:
        ...
        if stats_elem:
            ...
            if not view_elem:
                view_elem = stats_elem.find('span', text=lambda t: '浏览' in t)
            if view_elem:
                import re
            ...
            if not reply_elem:
                reply_elem = stats_elem.find('span', text=lambda t: '回复' in t)
            if reply_elem:
            ...
            if not like_elem:
                like_elem = stats_elem.find('span', text=lambda t: '点赞' in t)
            if like_elem:
