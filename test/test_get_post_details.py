    ...
    try:
        ...
        if author_elem:
            ...
            if name_elem:
                ...
                if '/u/' in href:
                    post_detail['author_uid'] = href.split('/u/')[-1]
...
def main():
    ...
    # 输入帖子ID文件
    input_file = input("请输入帖子ID文件路径（默认: user_5678597326_posts_direct.json）: ").strip()
    if not input_file:
        input_file = 'user_5678597326_posts_direct.json'
    ...
    for i, post in enumerate(posts, 1):
        ...
        print(f"\n[{i}/{len(posts)}]")
        detail = get_post_detail(post_id, cookie_str)
        if detail:
    ...
    # 保存详细信息
    if all_details:
        output_file = f"post_details_{int(time.time())}.json"
        ...
        for i, detail in enumerate(all_details, 1):
            print(f"\n{i}. ID: {detail['id']}")
            print(f"   标题: {detail['title'][:80]}")
            print(f"   作者: {detail['author_name']} (UID: {detail['author_uid']})")
    ...
    print(f"\n完成！共获取 {len(all_details)} 个帖子的详细信息")
