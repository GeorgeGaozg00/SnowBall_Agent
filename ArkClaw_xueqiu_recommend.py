...
# 4. 抓取推荐文章
def fetch_recommend_articles():
    """抓取雪球推荐文章，使用推荐流API"""
    global current_max_id, processed_articles
    ...
    apis = [
        {
            ...
            "params": {"since_id": "-1", "max_id": current_max_id if current_max_id else "-1", "count": 15, "source": "app"},
            "name": "个性化推荐流"
        },
        {
            "url": "https://xueqiu.com/v4/statuses/public_timeline_by_category.json",
            "params": {"since_id": "-1", "max_id": current_max_id if current_max_id else "-1", "count": 15, "category": "-1"},
            "name": "公开推荐流"
        }
    ]
...
# 7. 主循环
def main():
    global processed_articles, current_max_id
    ...
    while count < test_count:
        ...
        articles = fetch_recommend_articles()
        ...
        print(f"成功抓取到 {len(articles)} 篇新推荐文章")
    ...
    print("\n任务完成！")
    print(f"总共处理了 {len(processed_articles)} 篇不同的推荐文章")
